...
class PetfoodController:
    def __init__(self, page: ft.Page, popup):
        ...
        if self.storage.get(key="food_text"):
            self.food_picker_field.content.controls[0].value = self.storage.get(key="food_text")
    ...
    async def select_food(self, product_detail_id, product_name):
        ...
        try:
            ...
            self.food_picker_field.content.controls[0].value = f"{product_name} (용량 확인 중...)"
            self.page.update()
            ...
            if res_w.status_code == 200:
                ...
                if not weights_data:
                    self.food_picker_field.content.controls[0].value = f"{product_name} (선택 가능한 용량 없음)"
                    self.product_weight_list.visible = False
                    self.page.update()
                    return
                ...
                self.food_picker_field.content.controls[0].value = f"{product_name} (정보 조회 실패)"
                self.page.update()
    ...
    def food_product_weight_set(self, e):
        ...
        # 선택된 옵션에서 무게 값 추출
        selected_option = next((opt for opt in self.product_weight_list.options if opt.key == selected_product_id), None)
        if selected_option:
            try:
                weight_val = selected_option.text.replace("g", "")
                self.storage.set(key="product_weight", value=int(float(weight_val)))
                print(f"[DEBUG] 최종 저장: ProductID={selected_product_id}, Weight={weight_val}g")
...
def pet_food_view(page: ft.Page, popup):
    ...
    if storage.get("food_weight"):
        selected_food_weight.value = str(storage.get("food_weight"))
        
    # ---------------------------------------------------------------------------------------------------
    # Pet Feeding Food Page Layout
    # ---------------------------------------------------------------------------------------------------
    content_column = [
