...
def on_boarding_tile(page: ft.Page, content_page: str, change_page_callback):
    ...
    if content_page == "/sign_up":
        ...
        async def sign_up_next(e):
            if (
                ...
                and storage.get("user_name")
                and storage.get("user_password")
            ):
                if not re.fullmatch(
                    pattern=regex_email, string=storage.get("user_email")
                ):  # type: ignore
                ...
                try:
                    ...
                    if response.status_code == 201:
                        ...
                        access_token = resp_data.get("authorization", {}).get(
                            "access_token"
                        )
                        refresh_token = resp_data.get("authorization", {}).get(
                            "refresh_token"
                        )
                        if access_token:
                        ...
                        msg = (
                            response.json().get("detail", "회원가입 실패")
                            if response.content
                            else "회원가입 실패"
                        )
                        show_error(text=msg)
        ...
        bottom = ft.Row(controls=[dogdog.continue_button(on_click=sign_up_next)])
    # ---------------------------------------------------------------------------------------------------
    elif content_page == "/pet_info":

        async def pet_info_next(e):
            if (
                storage.get("breed_text")
                and storage.get("pet_name")
                and storage.get("pet_weight")
            ):
                if storage.get("pet_birth_day"):
                    ...
                    birth = str(
                        date.today() - relativedelta(months=age_month, years=age_year)
                    )
                else:
                ...
                Api_push_Data.data.update(
                    {
                        "nickname": storage.get("pet_name"),
                        "image_path": storage.get("image_path"),
                        "breed_id": storage.get("breed_id"),
                        "birth": birth,
                        "sex": sex,
                        "is_neutered": is_neutered,
                        "weight": storage.get("pet_weight"),
                    }
                )
                await focus_field.focus()
                page.update()
                ...
                Api_push_Data.data.update(
                    {
                        ...
                        "daily_walks": int(storage.get("radio_time")),  # type: ignore
                    }
                )
                await focus_field.focus()
                page.update()
    ...
    # ---------------------------------------------------------------------------------------------------
        ...
        async def pet_health_next(e):
            if storage.get("allergy"):
                Api_push_Data.data.update({"allergy": storage.get("allergy")})
            if storage.get("disease"):
                Api_push_Data.data.update({"disease": storage.get("disease")})
            ...
            try:
                ...
                if sex == 1 and not is_neutered:
                    san = 1
                elif sex == 2 and not is_neutered:
                    san = 2
                elif sex == 1 and is_neutered:
                    san = 3
                else:
                    san = 4
                ...
                if not isinstance(allergies_val, list):
                    allergies_val = [allergies_val] if allergies_val else []
                if not isinstance(diseases_val, list):
                    diseases_val = [diseases_val] if diseases_val else []
                ...
                payload = {
                    ...
                    "bcs": int(d.get("bcs", 3)),
                    "daily_walks": int(d.get("daily_walks", 0)),
                ...
                if res.status_code in [200, 201]:
                    res_data = res.json()
                    new_pet_id = res_data.get("data", {}).get(
    ...
    # ---------------------------------------------------------------------------------------------------
        ...
        async def pet_food_next(e):
            ...
            Api_push_Data.data.update(
                {"product_id": int(p_id), "food_weight": int(f_weight_num)}
            )
