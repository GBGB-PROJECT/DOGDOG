...
def main(page: ft.Page, on_complete=None):
    ...
    search_input = ft.TextField(
        label="도로명 주소, 건물명 검색", expand=True, autofocus=True
    )  # 찾기 창(input)
        ...
        try:
            ...
            if common.get("errorCode") != "0":
                ...
                result_column.controls.append(
                    ft.Text(error_message_common, size=15, color="red")
                )
                return
            ...
            if not page_juso:
                result_column.controls.append(
                    ft.Text("검색 결과가 없습니다.", size=15, color="red")
                )
            else:
            ...
            result_column.controls.clear()
            result_column.controls.append(
                ft.Text("시스템 오류입니다. 다시 검색하세요.", size=15, color="red")
            )
    ...
    async def on_address_select(e):
        ...
        async def comfirm_popup(click_e):
            ...
            result_column.controls.clear()
            result_column.controls.append(
                ft.Text("상세 주소를 입력하시고 [확인] 혹은 Enter를 눌러주세요!")
            )
            search_input.error_text = ""  # 선택 시점에 기존 에러내역을 지워줌
            result_column.controls.append(
                ft.Text(
                    f"선택하신 주소: \n{e.control.data.get('jibun')}",
                    weight="bold",
                    color="blue",
                )
            )
        ...
        check_popup.content = ft.Column(
            ...
            controls=[
                ft.Text(
                    f"{search_input.data.get('result2_name')}", weight="bold", size=16
                ),
        ...
        content=ft.Container(
            ...
            content=ft.Column(
                ...
                controls=[
                    ...
                    ft.Divider(),
                    ft.Text(
                        "아래와 같은 조합으로 검색하시면 결과를 가장 정확하게 찾을 수 있습니다.",
                        size=15,
                    ),
                    ...
                    ft.Button(
                        "확인했어요!",
                        on_click=lambda e: bottom_close(e),
                        width=float("inf"),
