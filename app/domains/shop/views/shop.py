...
def shop_feeding_guide(page: ft.Page):
    ...
    feeding_guide = ft.Column(
        ...
        controls=[
            ...
            dogdog.basic_text(
                "아침 39g, 저녁 39g", weight="bold", color=ft.Colors.GREY_600
            ),
...
def product_guide(page: ft.Page):
    ...
    from domains.shop.controller.shop_api import (
    ...
    def product_guide_page(key, e=None):
        ...
        if key == "forward":
            if product_guide.selected_index == product_guide.length - 1:
                ...
                product_guide.selected_index = view_page_index + 1
        elif key == "back":
    ...
    async def load_recommended_foods():
        ...
        if not foods:
            product_guide.content.controls[1].controls = [
                ft.Row(
                    ...
                    controls=[
                        dogdog.basic_text(
                            "추천사료가 없습니다.", size=12, color=ft.Colors.GREY_500
                        )
                    ],
                )
            ]
        ...
        recommended_views = dogdog.products(page, foods, guide_image_size)
    ...
    product_list = ft.Container(
        ...
        content=ft.Column(
            controls=[
                ft.Container(alignment=ft.Alignment.CENTER, content=ft.ProgressRing())
            ]
        ),
    )
    ...
    async def load_products(sort=None):
        ...
        if not products:
            product_list.content = ft.Column(
                controls=[
                    dogdog.basic_text(
                        "상품이 없습니다.", size=14, color=ft.Colors.GREY_500
                    )
                ]
            )
        else:
            print("상품 리스트 출력")
            product_list.content = ft.Column(
                controls=dogdog.products(page, products, product_image_size)
            )
    ...
    product_filter = dogdog.dropdown_menu(
        label=None, event=selected_filter, options=filter_list
    )
