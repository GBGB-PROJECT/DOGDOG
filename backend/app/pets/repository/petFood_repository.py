...
# ------------------------------ 수정 ------------------------------
# 수정 가능 날짜 조회
def get_pet_food_by_effective_date(db: Session, pet_id: int, effective_date: date):
    query = (
        select(CompanionPetProductFeeding).where(
            CompanionPetProductFeeding.pet_id == pet_id,
            ...
            (
                (
                    CompanionPetProductFeeding.feeding_false_date.is_(None)
                )  # 급여중이거나
                | (
                    CompanionPetProductFeeding.feeding_false_date >= effective_date
                )  # 급여 종료 날짜 전을 수정할 때
            ),
        )
...
def get_pet_foods_after_effective_date(db: Session, pet_id: int, effective_date: date):
    query = (
...
def close_pet_food_history(pet_food: CompanionPetProductFeeding, effective_date: date):
    pet_food.is_feeding_check = False
...
def deactivate_future_pet_foods(db: Session, pet_id: int, effective_date: date):
    future_foods = get_pet_foods_after_effective_date(
        db=db, pet_id=pet_id, effective_date=effective_date
    )
...
def update_same_day_pet_food(
    pet_food: CompanionPetProductFeeding, product_id: int, one_gram_calories: float
):
...
def upsert_customer_food_for_update(
    db: Session, pet_id: int, total_weight: int, effective_date: date
):
    customer_food = get_customer_food_id(db=db, pet_id=pet_id)
