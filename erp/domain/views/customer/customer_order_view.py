...
# 🔥 httpx 방식 API 호출로 변경
from api.erp_httpx_api import count_customer_orders, fetch_customer_orders, fetch_customer_orders_page
...
def erp_customer_order_view():
    ...
    def fetch_customer_order_page(keyword="", page_no=1):
        offset = (page_no - 1) * PAGE_SIZE
        start_date, end_date = get_selected_date_range()

        result = fetch_customer_orders_page(
            search_type=search_type_value["value"],
            keyword=keyword,
            limit=PAGE_SIZE,
            offset=offset,
        ...
        return {
            "rows": customer_order_db_row_adapter(result["items"], page_no),
    ...
    def reload_current_page():
        ...
        page_result = fetch_customer_order_page(keyword=keyword, page_no=current_page)
