...
def erp_employee_view():
    ...
    def update_reset_button_visibility():
        ...
        has_filter = (
            ...
            or search_type_value["value"] != "username"
    ...
    def on_reset(e):
        # 🔥 추가: defective_view.py와 동일하게 모든 조건을 기본값으로 복구
        search_field.value = ""
        selected_start["value"] = None
        selected_end["value"] = None
        search_type_value["value"] = "username"
        search_type_text.value = search_type_labels["username"]
        pagination_state["keyword"] = ""
        pagination_state["current_page"] = 1
        refresh_picker_fields()
        load_rows(e.page)
    ...
    search_field.on_submit = lambda e: on_search(e)
    ...
    action_controls = [
        action_button("조회", on_click=on_search, width=78),
        reset_button_holder,
